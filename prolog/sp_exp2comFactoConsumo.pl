...
cria_facto_consumo:-findall(X,facto(_,device(_,X)),LR),write(LR),calcula_consumo(LR,T),write(T),retract(ultimo_facto(N1)),
	N is N1+1,
	asserta(ultimo_facto(N)),assertz(facto(N+1,(facto_total_consumo(T)))).
